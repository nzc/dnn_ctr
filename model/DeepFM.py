...
class DeepFM(torch.nn.Module):
    ...
    def forward(self, Xi, Xv):
        ...
        if self.use_deep:
            if self.use_fm:
                ...
                deep_emb = torch.cat(ffm_wij_arr, 1)
            else:
        ...
        if self.use_fm and self.use_deep:
            total_sum = torch.sum(fm_first_order,1) + torch.sum(fm_second_order,1) + torch.sum(x_deep,1)
        elif self.use_ffm and self.use_deep:
            total_sum = torch.sum(ffm_first_order, 1) + torch.sum(ffm_second_order, 1) + torch.sum(x_deep, 1)
        elif self.use_fm:
            total_sum = torch.sum(fm_first_order, 1) + torch.sum(fm_second_order, 1)
        elif self.use_ffm:
            total_sum = torch.sum(ffm_first_order, 1) + torch.sum(ffm_second_order, 1)
        else:
    ...
    def fit(self, Xi_train, Xv_train, y_train, Xi_valid=None, Xv_valid=None,
                y_valid = None, ealry_stopping=False, refit=False):
        """
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range(batch_iter+1):
                ...
                if self.verbose:
                    if i % 100 == 99:  # print every 100 mini-batches
                        ...
                        print('[%d, %5d] loss: %.3f metric: %.3f time: %.1f s' %
            ...
            print('*'*50)
            print('[%d] loss: %.3f metric: %.3f time: %.1f s' %
